...
def mnist_extract_samples(sample_list, is_train=True, file_path=os.path.dirname(__file__)):
    ...
    s1, s2, s3, s4 = f_images.read(4), f_images.read(4), f_images.read(4), f_images.read(4) # 读4个字节
    mn_im = unpack('>I', s1)[0] # 将读取的二进制解包成元组
    ...
    for sample in sample_list:
        f_images.seek(16 + sample * rows_im * cols_im) # 定位到文件中多少个
