...
def weight_variable(shape):
    initial = tf.truncated_normal(shape, stddev=0.1) # 产生截断的正态分布，标准差0.1 产生的值如果与均值的差值大于两倍的标准差则丢弃重新选择
...
class ModelCNNSarrt(ModelCNNAbstract):
    def __init__(self):
        super().__init__()
        pass
    ...
    def create_graph(self, learning_rate=None):
        ...
        # self.x = tf.placeholder(tf.float32, shape=[None, 36864])
        # self.x = tf.placeholder(tf.float32, shape=[None, 32400]) # 为了SAR改的
        self.y_ = tf.placeholder(tf.float32, shape=[None, 10])
        self.x_image = tf.reshape(self.x, [-1, 32, 32, 4])
        self.W_conv1 = weight_variable([5, 5, 4, 32])
