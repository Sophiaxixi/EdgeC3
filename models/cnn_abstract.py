...
class ModelCNNAbstract(abc.ABC):
    ...
    def gradient(self, imgs, labels, w, sample_indices=None):
        if not self.graph_created:
            raise Exception('Graph is not created. Call create_graph() first.')

        if sample_indices is None:
            sample_indices = range(0,len(imgs))

        self.assign_flattened_weight(self.session, w)

        # 确保 labels 是 one-hot 编码的形式，即 [batch_size, 10]
        labels_one_hot = [labels[i] for i in sample_indices]
        ...
        grad_var_list = self.session.run(self.grad,
                                         feed_dict={self.x: [imgs[i] for i in sample_indices], self.y_: labels_one_hot})

        grad_flatten_list = []
        for l in grad_var_list:
            grad_flatten_list.append(np.reshape(l[0], l[0].size))

        grad_flatten_array = np.hstack(grad_flatten_list)# 水平方向合并 行不变
